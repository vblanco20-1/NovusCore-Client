...
void AuthSocket::AuthHandlers::Setup(MessageHandler* messageHandler)
{
    messageHandler->SetMessageHandler(Opcode::SMSG_SEND_ADDRESS, AuthSocket::AuthHandlers::SMSG_SEND_ADDRESS);
    messageHandler->SetMessageHandler(Opcode::SMSG_LOGON_CHALLENGE, AuthSocket::AuthHandlers::HandshakeHandler);
    messageHandler->SetMessageHandler(Opcode::SMSG_LOGON_HANDSHAKE, AuthSocket::AuthHandlers::HandshakeResponseHandler);
}
bool AuthSocket::AuthHandlers::SMSG_SEND_ADDRESS(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
{
    u32 address = 0;
    u16 port = 0;
    ...
    {
        // We need to implement the auth server at this point, once thats done we can uncomment below
...
bool AuthSocket::AuthHandlers::HandshakeHandler(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
{
...
bool AuthSocket::AuthHandlers::HandshakeResponseHandler(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
{
