...
namespace AuthSocket
{
    void AuthHandlers::Setup(MessageHandler* messageHandler)
    {
        ...
        messageHandler->SetMessageHandler(Opcode::SMSG_SEND_ADDRESS, { ConnectionStatus::AUTH_NONE, sizeof(u8), AuthHandlers::HandleSendAddress });
    }
    bool AuthHandlers::HandshakeHandler(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
    {
    ...
    bool AuthHandlers::HandshakeResponseHandler(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
    {
    ...
    bool AuthHandlers::HandleSendAddress(std::shared_ptr<NetworkClient> authSocket, NetworkPacket* packet)
    {
        u8 status = 0;
        u32 address = 0;
        u16 port = 0;
